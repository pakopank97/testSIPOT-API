...
def procesar_archivo_en_segundo_plano(filepath, task_id):
    ...
    try:
                ...
                if col_idx < len(headers) and str(headers[col_idx]).strip() != '':
                    ...
                    if pd.isna(valor) or str(valor).strip() == '':
                        ...
                        lista_de_errores.append(
                            f"Fila {fila_idx+1} eliminada. Celda {coordenada} bajo encabezado '{header_name}' estaba vacía."
                        )
                        ...
                        if regla in VALIDADORES:
                            ...
                            if not validador['func'](valor):
                                coordenada = obtener_coordenada_excel(fila_idx, col_idx)
                                lista_de_errores.append(
                                    f"Fila {fila_idx+1} eliminada. Celda {coordenada} ('{valor}') inválida. Se esperaba: {validador['nombre']}."
                                )
                                filas_invalidas.add(fila_idx)
                                break

        if lista_de_errores:
            # --- Mantener primeras 7 filas sin tocar ---
            df_header = df.iloc[:7]
            # --- Filas a validar (de la fila 8 en adelante) ---
            df_datos = df.iloc[7:]

            # --- Eliminar filas inválidas usando índices absolutos ---
            ...
            # --- Reconstruir Excel completo ---
            df_corregido = pd.concat([df_header, df_corregido_datos])

            # Guardar archivo corregido temporal
        ...
        tasks[task_id] = {
            'status': 'complete',
            'result': {'status': 'success', 'download_file': nombre_archivo_unico}
        }
...
#endpoint para verificar estado de la tarea
...
#endpoint para descargar archivo corregido o JSON (JSON solo si no hubo errores)
@app.route('/download/<filename>')
def descargar_archivo(filename):
    return send_from_directory(DOWNLOAD_FOLDER, filename, as_attachment=True)

# --- LIMPIEZA DE ARCHIVOS TEMPORALES pa no saturar el server ---
